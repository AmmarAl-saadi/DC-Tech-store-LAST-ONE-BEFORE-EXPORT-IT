    ...
    public function all(Request $request)
    {
        try {
            ...
            if ($lang) {
                ...
                }, 'public_sub_categories' => function ($q) use ($lang) {
                    $q->leftJoin('category_langs as cl', function ($join) use ($lang) {
                        $join->on('cl.category_id', '=', 'categories.id');
                        $join->where('cl.lang', $lang);
                    });
                    $q->select('categories.*', 'cl.title');
                }]);
                ...
                if ($request->category) {
                    $category = Category::with(['child', 'public_sub_categories'])
            ...
            if ($request->category) {
                if ($category) {
                    ...
                    if (isset($lang) && $lang) {
                        $subCatQuery = $subCatQuery->leftJoin('category_langs as cl', function ($join) use ($lang) {
                            $join->on('cl.category_id', '=', 'categories.id');
                            $join->where('cl.lang', $lang);
                    ...
                    if ($subCat) {
                        ...
                        // Category not found
                        $query = $query->where('products.id', -1);
                    }
                }
            }
    ...
    public function common(Request $request)
    {
        try {
            ...
            $commonData = Utils::cacheRemember($cacheKey, function () use ($request, $lang) {
                ...
                $data['slider_autoplay'] = Config::get('constants.SLIDER_AUTOPLAY');
