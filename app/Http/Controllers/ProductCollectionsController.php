...
class ProductCollectionsController extends ControllerHelper
{
    public function all(Request $request)
    {
        try {
            ...
            if ($lang) {
                ...
                if ($request->q) { $query = $query->where('pcl.title', 'LIKE', "%{$request->q}%"); }
            } else {
                if ($request->q) { $query = $query->where('product_collections.title', 'LIKE', "%{$request->q}%"); }
            }

            $data = $query->paginate(Config::get('constants.api.PAGINATION'));
            foreach ($data as $item) { $item['created'] = Utils::formatDate($item->created_at); }
    ...
    public function find(Request $request, $id)
    {
        try {
            $lang = $request->header('language');
            if ($can = Utils::userCan($this->user, 'product_collection.view')) { return $can; }
            $query = ProductCollection::query();
            if ($lang) {
                $query = $query->leftJoin('product_collection_langs as trl', function ($join) use ($lang) {
                    $join->on('trl.product_collection_id', '=', 'product_collections.id');
            ...
            $data = $query->find($id);
            if ($this->isVendor && $isOwner = Utils::isDataOwner($this->user, $data)) { return $isOwner; }
            if (is_null($data)) { return response()->json(Validation::noDataLang($lang)); }
            return response()->json(new Response($request->token, $data));
        } catch (\Exception $ex) {
            return response()->json(Validation::error($request->token, $ex->getMessage()));
        }
    ...
    public function action(Request $request, $id = null)
    {
        try {
            ...
            if ($id) {
                if ($can = Utils::userCan($this->user, 'product_collection.edit')) { return $can; }
                $existing = ProductCollection::find($id);
                if ($this->isVendor && $isOwner = Utils::isDataOwner($this->user, $existing)) { return $isOwner; }
                if ($bySlug && $bySlug['id'] != $id) { return response()->json(Validation::error($request->token, __('lang.slug_exists', [], $lang))); }
                $filtered = array_filter($request->all(), function ($element) { return '' !== trim($element); });

                if ($lang) {
                    [$langData, $mainData] = Utils::seperateLangData($filtered, ['title']);
                    ProductCollection::where('id', $id)->update($mainData);
                    $existingLang = ProductCollectionLang::where('product_collection_id', $id)->where('lang', $lang)->first();
                    if (!$existingLang) {
                        $langData['product_collection_id'] = $id; $langData['lang'] = $lang;
                ...
                if ($can = Utils::userCan($this->user, 'product_collection.create')) { return $can; }
                if ($bySlug) { return response()->json(Validation::error($request->token, __('lang.slug_exists', [], $lang))); }
                $request['admin_id'] = $request->user()->id;
                if ($lang) {
                    [$langData, $mainData] = Utils::seperateLangData($request->all(), ['title']);
                    $productCollection = ProductCollection::create($mainData);
                    $langData['product_collection_id'] = $productCollection->id; $langData['lang'] = $lang;
                    ProductCollectionLang::create($langData);
    ...
    public function delete(Request $request, $id)
    {
        try {
            $lang = $request->header('language');
            if ($can = Utils::userCan($this->user, 'product_collection.delete')) { return $can; }
            $ids = explode(",", $id);
            foreach ($ids as $i) {
                $productCollection = ProductCollection::find($i);
                if (is_null($productCollection)) { return response()->json(Validation::nothingFoundLang($lang)); }
                if ($this->isVendor && $isOwner = Utils::isDataOwner($this->user, $productCollection)) { return $isOwner; }
