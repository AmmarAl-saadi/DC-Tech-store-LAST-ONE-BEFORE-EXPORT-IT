  ...
  <div>
    ...
    <div class="flex gap-20 align-initial wrap start" v-if="hasRadio">
      ...
      >
        ...
        <span class="block mt-5">{{ $t('invent.tel', {phone: value.phone}) }}</span>
      </label>

      <button
        class="address-btn card" type="button"
        @click.prevent="emitAddAddress"
      >
...
<script setup>
  ...
  const emit = defineEmits(['selected-address', 'add-address']);

  const emitAddAddress = () => {
    emit('add-address')
  };
  ...
  const selectLatest = (recentAddress) => {
    const index = currentAddresses.value?.findIndex(i => {
      return i?.id === recentAddress?.id
    });
  ...
  defineExpose({selectLatest, loadData})
