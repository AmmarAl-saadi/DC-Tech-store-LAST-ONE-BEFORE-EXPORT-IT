<template>
  <div class="embla glide slider-wrapper" ref="rootRef">
    ...
    <div class="embla__buttons glide-nav">
      <button @click="scrollPrev" class="prev-btn" :class="!activation.prev && 'disabled'"><i
        class="m-0 icon arrow-left"></i></button>
      <button @click="scrollNext" class="next-btn" :class="!activation.next && 'disabled'"><i
        class="m-0 icon arrow-right"></i></button>
    </div>
...
<script setup>
  ...
  const plugins =
    autoplay.value > 0
      ? [Autoplay({delay: autoplay.value, stopOnInteraction: false})]
      : []
  ...
  watch(emblaApi, (api) => {
    ...
    api.on("pointerUp", () => api.plugins().autoplay?.play())
  ...
  onMounted(() => {
    if (!emblaApi.value || autoplay.value <= 0) return

    const autoplayPlugin = emblaApi.value.plugins().autoplay

    rootRef.value.addEventListener("mouseenter", autoplayPlugin.stop)
    rootRef.value.addEventListener("mouseleave", autoplayPlugin.play)
  })
