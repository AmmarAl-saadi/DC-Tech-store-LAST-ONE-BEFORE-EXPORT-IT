  ...
  export default {
    setup() {
      ...
      return {
        ...
        currency, paymentGateway, site_setting, bgGetRequest,
        currencyIcon, setting, setToastMessage, setToastError,
        getCartByUser, subtractCartProductCount, emptyCartProduct
      }
    },
    middleware: ['auth'],
    data() {
    ...
    methods: {
      ...
      orderPlaced(type = 'success', event, redirect = true, showToast = true) {
        if (type === 'success') {
          ...
          if (redirect) {
            this.$router.push({path: '/profile/order/' + event})
          }
          ...
          this.$router.push({path: '/profile/order/' + this.orderId})
