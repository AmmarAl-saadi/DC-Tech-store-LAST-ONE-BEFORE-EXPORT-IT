...
<script setup>
    ...
    const loginWith = async (service) => {
        window.location.href = await socialRedirect(service);
    };

    const userStore = useUserStore();
    const {getUserToken} = userStore;

    const socialRedirect = async (service) => {
        const config = useRuntimeConfig();
        const baseUrl = config.public.apiBase;
        const userToken = await getUserToken();

        const apiBase = !baseUrl.trim() ? window.location.origin + '/' : baseUrl;
        return `${apiBase}${json.api.url}${json.api.socialLogin}/${service}?user_token=${userToken}`;
    };
