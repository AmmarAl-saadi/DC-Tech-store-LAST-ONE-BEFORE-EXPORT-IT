...
class AdminsController extends Controller
{
    ...
    public function profile(Request $request)
    {
        try {
            ...
            if ($isLocalhost) {
                ...
                $currentDomain = Utils::normalizeDomain($domain);
                $licenceDomain = Utils::normalizeDomain($validLicence->d);

                if ($validLicence && $licenceDomain === $currentDomain) {
                    $data['activated'] = true;
                    $data['public_key'] = $licence->public_key;
                }
            }
    ...
    public function delete(Request $request, $id)
    {
        try {
            ...
            $category = Category::where('admin_id', $id)->get()->first();
            if ($category) {
                return response()->json(Validation::error($request->token,
                    __('lang.has_categories', [], $lang)));
            }
